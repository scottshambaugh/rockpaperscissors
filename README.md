...
- [Labeled counts and the domination-number distribution](#labeled-counts-and-the-domination-number-distribution)
...
Each fairness cell - and the two-paradox cell through `n=11` - is `total [prime]`: the total count up to isomorphism and the modular-prime subcount in brackets. **Past the brute-force range the `[prime]` breakdown isn't computed, so those cells show the total only** (e.g. `n ≥ 12` balanced). `-` means not reached.
...
The `two-paradox` column is the **raw S₂** tournament count (`A000568`-restricted; the paradoxical+connected sub-count is the slightly smaller `1, 4, 221` at `n=7,8,9` - see the OEIS notes), now also given as `total [prime]`: a two-paradox tournament *can* be modular-prime, and the large majority are (the `n=7` one is the Paley tournament, prime and rigid) - but genuine non-prime S₂ tournaments exist (`2` at `n=8`, `2087` at `n=10`), so the distinction is real. Prime subcounts are computed by nauty enumeration + modular-decomposition through `n=11`; past that (`n ≥ 12`) the cell is total-only. Counts past the brute-force wall come from nauty + the tools in [`nauty/`](nauty/) and [`rust/`](rust/), each anchored to a known checksum (`A001174` / `A000568` / `A308239`).

| n  | candidates = 3^C(n,2)            | iso classes             | two-paradox | regular                  | balanced                 | inclusive                   | completely mixed     | n  |
|----|----------------------------------|-------------------------|-------------|--------------------------|--------------------------|-----------------------------|----------------------| ---- |
| 3  | 27                               | 7                       | 0           | 1 [1]                | 1 [1]                | 1 [1]                   | 1 [1]            | 3 |
| 4  | 729                              | 42                      | 0           | 1 [1]                | 1 [1]                | 3 [2]                   | 0                    | 4 |
| 5  | 59049                            | 582                     | 0           | 2 [2]                | 4 [3]                | 15 [7]                  | 7 [6]            | 5 |
| 6  | 14348907                         | 21480                   | 0           | 5 [4]                | 16 [13]             | 222 [169]             | 0                    | 6 |
| 7  | 10460353203                      | 2142288                 | 1 [1]       | 13 [12]             | 175 [152]          | 10525 [9350]         | 7268 [7240]   | 7 |
| 8  | 22876792454961                   | 575016219               | 5 [3]       | 82 [76]             | 5274 [4917]       | 1198013 [1127592] | 0                    | 8 |
| 9  | 150094635296999121               | 415939243032            | 226 [197]   | 2016 [1972]       | 434017 [420437] | 654809211 [645436603] | 583591020 [583533123] | 9 |
| 10 | 2954312706550833698643           | 816007449011040         | 29816 [27729] | 154831 [153529] | 90658149 [89432057] | 360415933231                | 0                    | 10 |
| 11 | 174449211009120179071170507      | 4374406209970747314     | 6959159 [6639506] | 21171976 [21091694] | 48825116761 [48546116371] | -                           | -                    | 11 |
| 12 | 30903154382632612361920641803529 | 64539836938720749739356 | 2629321652  | 9348286118 [9332883596] | 68579602126387           | -                           | 0                    | 12 |
| 13 | 16423203268260658146231467800709255289 | 2637796735571225009053373136 | - | 12050109962241 | 254236892409251687 | - | - | 13 |
| 14 | 26183890704263137277674192438430182020124347 | 300365896158980530053498490893399 | - | 26133517897247816 | 2512196983110257299898 | - | 0 | 14 |
| 15 | 125236737537878753441860054533045969266612127846243 | 95772686348981154949990837579207581003 | - | 193789800287451697002 | 66738846701458593981832314 | - | - | 15 |
| 16 | 1797010299914431210413179829509605039731475627537851106401 | 85888418249161165461289338402279023247144414 | - | 4477753123613209191571206 | 4802164152196181961357340585741 | - | 0 | 16 |

## Labeled counts and the domination-number distribution

The `two-paradox` column above counts games **up to isomorphism**. A separate, deeper quantity is the **labeled** count `L(n)` — the number of two-paradox (γ ≥ 3) tournaments on a *fixed* vertex set `[n]`. The iso count is `L(n)` collapsed by the `Sₙ` relabeling action (Burnside), so `L(n)` is **not** a column of the table above and is much larger: e.g. `L(11) = 277,644,146,489,600` while `iso(11) = 6,959,159`.

Two facts make `L(n)` worth recording on its own:

- By Reid–McRae–Hedetniemi–Hedetniemi, `γ(T) ≤ 3` for every tournament on `n < 19` vertices, so for `n ≤ 18` the domination number takes only values `{1, 2, 3}` and `L(n) = #{γ = 3}`. The full distribution is then `#{γ=1} + #{γ=2} + #{γ=3} = 2^C(n,2)`, with the closed form `#{γ=1} = n·2^C(n-1,2)` (one dominating "source" vertex) and `#{γ=2} = 2^C(n,2) − #{γ=1} − L(n)`. (`γ ≥ 4` first occurs at `n = 19`, so for `n ≥ 19`, `L(n)` counts `γ ≥ 3` rather than `γ = 3`.)
- `L(n)` is computed in **polynomial time**, **proved correct** with no remaining assumption. An inclusion–exclusion over dominating pairs has only caterpillar-forest and odd-cycle-"sun" terms (a domination-graph structure theorem of Fisher–Lundgren–Merz–Reid); each term's per-graph weight is given by a proved-exact transfer recurrence (the `Within` matrix — Theorem L2 — via a bipartite "two-colouring collapse"), and a symmetric-power / exponential-generating-function aggregation (Theorem L4) evaluates the whole signed sum *without enumerating the graphs*, in `poly(n)` big-integer operations — versus the `2^C(n,2)` brute force. The aggregation's exactness is proved (symmetric count-collapse, EGF automorphism bookkeeping, cyclic-seam closure); it reproduces `L(7)…L(16)` exactly as independent corroboration. (Summing the same weights by naïve enumeration instead is a `2^O(n)` special case, not used.) Full write-up (statements, proofs — including the L1/L2/L3 per-graph proofs and the L4 aggregation proof — and the `n=7` worked example): [`docs/two-paradox-counting.md`](docs/two-paradox-counting.md) ([PDF](docs/two-paradox-counting.pdf)). Implementations: [`rust/two_paradox_poly.rs`](rust/two_paradox_poly.rs) (polynomial), [`rust/two_paradox_count.rs`](rust/two_paradox_count.rs) (naïve-enumeration variant), both built on the `Within` primitive [`rust/caterpillar_within.rs`](rust/caterpillar_within.rs).

`L(n) = 0` for `n ≤ 6` (every small tournament has γ ≤ 2). For `n = 7 … 18`:

| n  | γ=1 = n·2^C(n-1,2) | γ=2 | γ=3 = L(n) |
|---:|---:|---:|---:|
...
| 15 | 37138201178561408246973726720 | 36763381828671744627985302636544 | 3764299177453034811662226208768 |
| 16 | 649037107316853453566312041152512 | 1146122823458354595888651195696381952 | 182456135219244423561589552542810112 |
| 17 | 22596875928343569839364720024765857792 | 70477931698709529153536730062502470615040 | 16611757357122373923247804720005425659904 |
| 18 | 1568021146771684439639230191045587918389248 | 8546059988433011215799933490851286631811186688 | 2870353532067896148226715034553628871332397056 |

**Status.** `L(7) … L(10)` agree with direct brute-force enumeration of all `2^C(n,2)` tournaments; `L(11)` is corroborated by a Burnside congruence against the known `iso(11)`. Values `L(12) … L(18)` come from the method above, whose per-caterpillar `Within` primitive is exhaustively validated (zero mismatches for all caterpillars up to `k ≈ 14`) but **not yet proved exact for all caterpillars** — so `L(12)…L(18)` should be read as validated computational results, not settled theorems.
...
`inclusive(10) = 360415933231` is out of reach for *enumeration* (the classes themselves are `~10¹¹`, multi-TB before dedup), so it was **counted without enumerating** (`rust/inc_census.sh`): Burnside's lemma `inclusive(n) = (Σ_σ Fix_σ)/n!`, with the identity term `L_inc(n)` split by matrix nullity into strata `L_D` (each counted by an exact grandparent→parent→child extension DFS) and the non-identity cycle types handled by sweeps/brutes. The `/n!` integrality gate is the correctness check. **The subtlety that makes this hard:** `L_inc(n)` is the sum of *every* even nullity stratum `L₂ + L₄ + ⋯ + L_{n−2}`, not just `L₂+L₄+L₆` - a rank-2 inclusive game (the RPS blow-ups) has nullity `n−2`, so that family sits in `L₆` at n=8 but **migrates to `L₈` at n=10** (`L₈ = 8884050`). Omitting it undercounts and the integrality gate fails; the modular-fingerprint diagnostic in `sigma_gate.py` (a residual prime factor `> n` ⇒ the coefficient-1 identity term, i.e. a missing stratum) pinpoints it. The whole count path is exact integer (no floating point), width-16 (`n ≤ 16`), with `-C overflow-checks=on` so any overflow fails hard.
